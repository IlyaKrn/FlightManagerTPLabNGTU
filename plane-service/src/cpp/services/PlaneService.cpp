...
list<PlaneModelResponse> PlaneService::getAllPlanes(string token)
{
    ...
    {
        PlaneModelResponse planeResponse(plane.getId(), plane.getName(), plane.getPilot(), plane.getBuiltYear(),
                                         plane.getBrokenPercentage(), plane.getSpeed(), plane.getMinAirportSize(), 0,
                                         0);
...
PlaneModelResponse PlaneService::getPlaneById(long int id, string token)
{
    ...
    PlaneModelResponse planeResponse(planes.front().getId(), planes.front().getName(), planes.front().getPilot(),
                                     planes.front().getBuiltYear(), planes.front().getBrokenPercentage(),
                                     planes.front().getSpeed(), planes.front().getMinAirportSize(), 0, 0);
...
bool PlaneService::updatePlane(PlaneModel plane, set<string> update, string token)
{
    ...
    PlaneModel n_plane(nplane.getId(), nplane.getName(), nplane.getPilot(), nplane.getBuiltYear(),
                       nplane.getBrokenPercentage(), nplane.getSpeed(), nplane.getMinAirportSize());
